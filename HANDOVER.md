...
## 2026-07-31 — PLAN.md 스테일 정리 (문서 작업만)

코드는 건드리지 않았다. `PLAN.md`를 읽다가 앞부분 설계와 뒷부분 결정이 서로
어긋나는 곳을 발견해 그것만 정리했다.

### 한 일

문제는 이랬다. 1~5절은 2026-07-25 설계 당시 스냅샷이고, 7~9·11~12절이 그 뒤에
내린 결정이다. Life 섹션을 철회한 뒤에도 앞절의 Life 기술이 그대로 남아 있어
처음 읽는 사람은 어느 쪽이 유효한지 알 수 없었다.

내용을 지우지 않고 철회 표시만 달았다. 왜 만들었다가 왜 접었는지가 남는 편이
낫다는 판단이고, Life Map 커밋(`f47351f`, `d4ab43a`)을 되돌리지 않은 것과 같은
논리다.

- 문서 앞머리에 읽는 순서 안내 추가 — 1~5절은 스냅샷, 7~9·11~12절이 현재 결정
- Life 관련 8곳에 철회 표시: 0절 항목3, 3.2 디렉터리, 3.3 스키마, 3.6 Life Map,
  Phase 1, Phase 4, 7절 사이드바, 9절 "Life에 무엇을 넣는가", 13절 4·6·7번
- 7절 사이드바는 철회 표시만으로 부족해서 현재 구현된 구성을 아래에 따로 적었다
- Phase 2 갱신 — 발행 노트 6개 → 4개(07-28 발행 취소 반영),
  `first_paragraph()` 자동 추출 제거 반영
- 12.4에 완료 표시와 요약 추가
- 10절이 결번임을 명시

`PLAN.md` 커밋 `f82fee2`. 푸시는 안 했다.

### 결정 사항

절 번호를 재정렬하지 않았다
- 9절 다음이 11절이라 당기고 싶었지만 `HANDOVER.md`가 `PLAN 11`, `PLAN 12.2`
  형태로 15곳에서 참조하고 있다. 당기면 그 참조가 전부 한 칸씩 어긋난다
- 결번을 명시하는 쪽이 싸다. 10절 자리에 그 이유를 적어뒀다

9절 "Life에 무엇을 넣는가"는 지우지 않고 네이버용으로 재배치
- 독서 감상, 아이들과 한 일, 본 것, 몸의 기록, 종교 — 콘텐츠 후보 목록 자체는
  네이버 블로그 소재로 여전히 쓸모가 있다
- 헤딩을 "menosaint가 아니라 네이버 블로그"로 바꾸고 스키마 초안에만
  미구현 표시를 달았다

13절 열린 결정 6·7번(종교, 몸의 기록)은 닫은 게 아니라 이관했다
- menosaint에 무엇을 올릴지의 문제가 아니게 됐을 뿐, 네이버에 무엇을 쓸지는
  여전히 정해지지 않았다. 이 문서의 결정 사항이 아니라고만 적었다

### 통한 접근

고치기 전에 참조 범위를 먼저 봤다
- `grep -rn "PLAN [0-9]\|PLAN 1[0-9]"`로 절 번호 참조 15곳을 먼저 찾았다
- 이걸 안 봤으면 번호를 당겼을 것이고 HANDOVER 참조가 전부 깨졌을 것이다
- 문서 구조를 바꾸기 전에 누가 그 구조를 참조하는지 세는 것이 먼저다

문서 기술을 코드로 검증했다
- `sync.py`에 `first_paragraph`가 정말 없는지, 노트가 정말 4개인지 grep과 ls로 확인
- PLAN에 적힌 내용을 그대로 믿고 고쳤으면 틀린 것을 틀리게 고쳤을 수 있다

### 주의할 점

세션 중간에 `HANDOVER.md`가 바뀌어 있었다
- 세션 시작 때 읽은 것은 349줄이었는데 나중에 grep해보니 378줄이었다.
  07-28 세션 항목이 그 사이에 들어와 있었다
- 편집하기 직전에 현재 상태를 다시 읽는 편이 안전하다. 세션 초반에 읽은
  내용을 그대로 믿고 덮어쓰면 남의 작업을 지운다
...
### PLAN.md의 절 번호를 재정렬하지 말 것

10절이 결번이라 당기고 싶어지지만, 이 문서가 `PLAN 11`, `PLAN 12.2` 형태로
15곳에서 참조한다. 새 절은 뒤에 붙이고 기존 번호는 고정한다.

### PLAN.md 앞부분은 설계 스냅샷이다

1~5절은 2026-07-25 시점의 구상이고 7~9·11~12절이 현재 결정이다. 충돌하는 곳에는
철회 표시를 달아뒀지만, 새로 읽을 때는 뒷절을 먼저 믿는 편이 안전하다.

---

## 다음 단계

`PLAN.md` 12절에 상세히 적혀 있다. 껍데기(12.4)를 먼저 끝냈으므로 남은 순서는 아래.

1. 설명 문구 재작성 (PLAN 12.5)
   - 히어로, 섹션 description, Knowledge 7개 영역 설명, `site.description`, OG
   - 지금 문장은 대부분 Claude가 쓴 것이다. 본인 목소리로 바꾼다
2. 라이트 · 다크 모드 (PLAN 12.6)
   - `global.css` 토큰을 `prefers-color-scheme` + 수동 토글 양쪽에 대응
   - localStorage 저장, FOUC 방지, 히트맵 4단계 색상은 다크에서 따로
3. 발행 전 확인 절차 구현 (PLAN 12.2, 안전장치)
   - `sync.py`에 검사 모드. 신규 발행 노트 목록 + 민감 패턴 스캔 + 중단 게이트
   - 07-28에 실명 노출을 손으로 찾아냈다. 스캔이 있었으면 sync에서 걸렸다
4. vault 정리와 발행 워크플로 확정 (PLAN 12.1)
...
6. 프로덕션 전환 (PLAN 11절. 캐시 퍼지 필수)

완료: 웹페이지 세부 정리 (PLAN 12.4, 07-28).
미완 항목: RSS, Expressive Code, 첨부 이미지 처리, Projects 섹션 구현.
발행 취소한 온톨로지 노트 2개는 실명을 지우면 되살릴 수 있다.
...
이후 커밋은 각 세션 항목 안에 있다. 07-28 5개, 07-31 1개(`f82fee2`).
