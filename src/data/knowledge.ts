 ...
 *
 * 매칭은 세 경로를 쓴다. vault 태그 체계가 완전히 정착되지 않아 제목이 실질적인
 * 분류자 역할을 하기 때문이다. 접두사(`인문 - `, `Claude Code - `)뿐 아니라
 * 제목 키워드도 본다. 온톨로지 노트들이 `domain/ai` 태그만 달고 있어서
 * 태그만으로는 지식 구조 영역이 비어 버렸다.
 */
...
export type KnowledgeArea = {
  ...
  /** 제목에 이 단어가 들어가면 이 영역. 태그가 안 붙은 노트를 건진다 */
...
export const knowledgeAreas: KnowledgeArea[] = [
  ...
  },
  {
    ...
    titleKeywords: ["온톨로지", "택소노미", "Ontology", "지식관리", "구조적 사고", "핸드오버"],
    topics: [
      "온톨로지와 택소노미",
      "조직지식관리 온톨로지 설계",
      "구조적 사고",
  ...
  },
  {
    ...
    titleKeywords: ["KPI", "OKR", "성과급", "보상", "RACI", "SIPOC", "프로세스 매핑", "기획"],
    topics: [
      "KPI와 OKR의 설계 철학",
      "보상 철학과 성과급의 역설",
      "RACI · SIPOC",
  ...
  },
  {
    ...
    tags: ["domain/legal"],
    titleKeywords: ["주주총회", "이사회", "상법", "근로", "베스팅", "스톡옵션"],
    topics: [
      "이사의 충실의무",
      "주주총회 결의사항",
      "스톡옵션과 베스팅",
  ...
  },
  {
    ...
    titleKeywords: ["Git", "터미널", "tmux", "cmux", "셸", "Shell", "CLI"],
 ...
 * 태그, 제목 접두사, 제목 키워드를 모두 본다. 복수 영역에 걸칠 수 있다.
 */
export function matchAreas(note: { title: string; tags?: string[] }): KnowledgeArea[] {
  ...
  return knowledgeAreas.filter((area) => {
    ...
    const byPrefix = (area.titlePrefixes ?? []).some((p) => note.title.startsWith(p))
    const byKeyword = (area.titleKeywords ?? []).some((k) =>
      title.includes(k.toLowerCase()),
    )
    return byTag || byPrefix || byKeyword
  })
}
