...
`v4` → `v5` 컷오버는 두 단계다. Pages 프로젝트의 production branch를 `v5`로 바꾸고,
`v5`에 푸시해 프로덕션 배포를 한 번 만든다. 커스텀 도메인은 항상 production branch를
따라가므로 DNS는 건드리지 않는다.

### 캐시 퍼지가 필요하다

Cloudflare 엣지 캐시가 오래 산다. 프리뷰에서 `/life/`를 지운 뒤에도 엣지가
`cf-cache-status: HIT`로 옛 페이지를 200으로 계속 돌려줬다. 캐시를 우회하면 정상적으로
404가 떴으니 원본은 맞았고 순전히 캐시 문제였다.

프로덕션 전환에서는 이게 더 크게 걸린다. v4(Quartz)의 URL 구조가 v5와 다르고,
`/lab/`, 개별 노트 경로, `index.css` 같은 자산이 전부 옛 내용으로 캐시되어 있다.
퍼지하지 않으면 사람마다 다른 버전을 보게 된다.

절차:

1. production branch를 `v5`로 변경
2. 프로덕션 배포 성공 확인
3. 전체 캐시 퍼지 — 대시보드 Caching → Configuration → Purge Everything.
   API는 `POST /zones/{zone_id}/purge_cache` 에 `{"purge_everything":true}`
4. 캐시 우회 요청(`?cb=난수`)이 아니라 순수 URL로 검증한다. 우회하면 캐시 문제를
   못 잡는다
5. v4의 주요 URL이 404를 반환하는지 확인. 살려야 할 URL은 `public/_redirects`로
   넘긴다

되돌리기: production branch를 `v4`로 돌리고 빌드 설정을 `npx quartz build` / `public`
으로 복구한 뒤 다시 퍼지한다. v4 브랜치와 그 시절 배포가 Cloudflare에 남아 있다.

## 12. 다음 작업 (2026-07-27 기준)

### 12.1 vault 정리와 발행 워크플로 확정

배포 방식이 정해졌으니 vault를 거기에 맞춘다.

- 노트 기본 템플릿(`9. templates/`)에 발행 관련 필드를 넣을지 검토.
  최소한 `publish`가 무엇을 뜻하는지가 템플릿에 드러나야 한다
- 제목 규약 정리. 현재 `인문 - `, `Claude Code - `, `R1. ` 같은 접두사가 실질적인
  분류자다. 태그로 수렴시킬지 접두사를 정식 규약으로 승격할지 정한다
- h1 표기 흔들림. `# agent - ...`처럼 소문자로 시작하는 h1이 있고 그대로 제목이 된다
- `1. Memory/` 외 폴더에서 발행할 일이 있는지 확인

### 12.2 발행 전 확인 절차 (안전장치)

know 폴더에 `연봉 삭감`, `경영성과급 설계`, `R1. 시장조사 소스` 같은 회사·인사
자료가 섞여 있다. `publish: true` opt-in 게이트가 1차 방어선이지만 실수로 켤 수 있다.
두 번째 방어선을 만든다.

`sync.py`에 검사 모드를 붙인다.

- 이번에 새로 발행되는 노트를 먼저 나열한다. 이미 나가 있던 것과 구분한다
- 민감 패턴을 스캔해 경고한다. 실명, 회사명, 이메일, 금액, 병원·기관명,
  사내 시스템 이름, 파일 경로(`/Users/...`)
- 경고가 있으면 기본적으로 중단하고, 확인 후에만 진행한다
- 결과를 사람이 읽을 수 있는 리포트로 출력한다

### 12.3 private.menosaint.xyz

이미 살아 있다. `menosaint-private` Pages 프로젝트이고 루트가 302로 인증을 태운다.

- 간단한 관리자 페이지 — 발행 상태, 최근 동기화, 미발행 노트 목록 같은 운영 정보
...
현재 사이트의 설명 문장은 대부분 Claude가 임의로 쓴 것이다. 본인 목소리로 바꾼다.

- 히어로 문구
- 각 섹션 description
- Knowledge 7개 영역의 한 줄 설명
- Experience 요약과 담당 영역 서술
- `site.description`, OG 설명

### 12.6 라이트 · 다크 모드

- `global.css`의 토큰을 `prefers-color-scheme`과 수동 토글 양쪽에 대응시킨다
- 토글 상태를 localStorage에 저장하고 FOUC를 막는다
- 히트맵 4단계 색상은 다크에서 따로 잡아야 한다
